		...
		<view class="head flex flex-direction">
			...
			<view class="flex justify-between head-top text-xl margin-top-sm" :style="{ paddingTop: paddingTop }">
				<view class="flex justify-between margin-left-sm head-left">
					...
					<view :class="tabNum == 1 ? 'bold-style text-bold' : ''" @tap="tabClick(1)">ToDo</view>
				</view>
			</view>
			<u-search
				...
				bgColor="#fff"
				height="80rpx"
				:placeholder="tabNum == 0 ? '搜索日记' : '搜索ToDo'"
				v-model="keyWord"
			></u-search>
		</view>
		<!-- 内容区域 -->
				...
				<scroll-view scroll-y style="height: 100%;width: 100%;">
					<view class="padding-bottom padding-lr">
						...
						<view class="bg-white radius padding-sm noteList" v-else>
							<view class="text-bold text-lg">你好，欢迎使用</view>
							<view class="flex justify-between margin-top-xs">
	...
	methods: {
		...
		addAgencyConfirm() {
			if (!this.agencyInfo) {
				...
				this.$refs.uToast.error('添加失败，ToDo不能为空！');
				return;
			}
		...
		add() {
			...
			if (this.tabNum == 0) {
				// 出现日记的内容，md文件内容
				uni.$u.route('/pages/addNote/addNote');
		...
		// 点击单条日记进行跳转
		noteListClick(e) {
			uni.$u.route('/pages/addNote/addNote', e);
		},
		menuClick(index) {
